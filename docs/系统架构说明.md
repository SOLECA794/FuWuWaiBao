...
### 1.3 设计亮点（用于体现“算法/架构创新性”与 AI 融合深度）

> 本节不是口号，而是“可落到输出字段/可验证行为”的设计亮点清单。

- **课件从静态到可执行：页 → 节点（node）化脚本**
  - 输出不是一段长文本，而是 `nodes[]`（opening/explain/transition）+ `node_id`。
  - 作用：支持高亮播放、被打断后精确续接到节点级。

- **分层检索 + 溯源（Grounded QA）**
  - 问答输出必须带 `source_page`/`source_section`，可在前端展示“答案来自第 X 页”。
  - 作用：降低幻觉，支撑“答案准确率 ≥ 85%”的验收口径。

- **意图识别驱动的教学策略（非普通聊天）**
  - QA 输出包含 `need_reteach` 与 `understandingLevel`（none/partial/full）。
  - 作用：将问答结果转化为“继续/补充/放慢/重讲”的节奏策略。

- **SSE 流式问答 + 分句事件**
  - SSE 帧支持 `token/sentence/final/error`。
  - 作用：用“首 token 快返回 + 分句 TTS”满足 5 秒体感指标。

- **泛雅 OpenAPI 适配层（兼容性可审计）**
  - 支持 `/api/v1/*` 标准路径、`enc` 签名、`requestId`、统一响应 `msg` 字段。
  - 作用：对接泛雅时无需改内部业务接口，提供可扩展标准化外部接口。
...
| OpenAPI Adapter | Go | 泛雅标准请求（enc+time） | 内部转调 | 签名校验、路径映射、响应格式统一（Fanya 风格） |
...
- **内部产品 API**：服务于教师端/学生端前端，字段更贴合前端使用（见 `API_DESIGN_V2.md` 第 2~7 章）。
- **泛雅开放 API（OpenAPI Adapter）**：服务于泛雅平台对接，严格遵循签名 `enc`、统一响应 `msg/requestId`（见 `API_DESIGN_V2.md` 第 8 章与 `泛雅API规范.md`）。

### 3.3.2 签名校验与安全边界

- 泛雅调用开放接口时：必须携带 `time` 与 `enc`，服务端按规范重算并校验，不通过返回 403。
- 内部前端调用内部 API：可使用 Bearer Token（或后续接入泛雅的 SSO/统一身份后再统一）。

### 3.3.3 异步任务对接（解析/生成）

- `lesson/parse` 与 `lesson/generateScript` 属于耗时能力，开放接口返回 `parseId/scriptId` 与 `taskStatus`。
- 泛雅侧可采用“轮询查询状态”或“回调通知”（按对接双方约定扩展）。
