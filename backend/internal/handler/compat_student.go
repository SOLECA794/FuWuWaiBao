...
func (h *CompatibilityHandler) StartStudentSession(c *gin.Context) {
	...
	apiresp.OK(c, "请求成功", gin.H{"sessionId": state.SessionID, "courseId": req.CourseID})
}
...
func (h *CompatibilityHandler) UpdateStudentProgress(c *gin.Context) {
	...
	apiresp.OK(c, "ok", gin.H{"sessionId": state.SessionID, "page": page, "nodeId": nodeID})
}
...
func (h *CompatibilityHandler) GetStudentScript(c *gin.Context) {
	...
	apiresp.OK(c, "请求成功", gin.H{"courseId": courseID, "page": page, "nodes": nodes, "page_summary": pageSummary, "audio_meta": audioMeta, "playback_mode": audioMeta["playback_mode"]})
}
...
func (h *CompatibilityHandler) GetStudentPlaybackAudio(c *gin.Context) {
	...
	apiresp.OK(c, "请求成功", buildPlaybackAudioMeta(courseID, page, nodes))
}
...
func (h *CompatibilityHandler) GetWeakPointsV1(c *gin.Context) {
	...
	if studentID == "" {
		apiresp.BadRequest(c, "缺少 studentId", "")
		return
	}
	apiresp.OK(c, "请求成功", h.listWeakPoints(studentID, courseID))
}
...
func (h *CompatibilityHandler) ExplainWeakPointV1(c *gin.Context) {
	...
	if err != nil {
		apiresp.ServiceUnavailable(c, "AI讲解暂不可用", "")
		return
	}
	apiresp.OK(c, "请求成功", gin.H{"weakPointId": weakPointID, "title": name, "content": resp.Answer})
}
...
func (h *CompatibilityHandler) GenerateWeakPointTestV1(c *gin.Context) {
	...
	if err != nil {
		apiresp.ServiceUnavailable(c, "AI习题生成失败", "")
		return
	}
	testData, err := parseWeakPointTest(resp.Answer)
	if err != nil {
		apiresp.Internal(c, "AI习题返回格式异常", "")
		return
	}
	...
	apiresp.OK(c, "请求成功", gin.H{"questionId": questionRecord.ID, "content": testData.Content, "options": testData.Options, "type": questionRecord.QuestionType})
}
...
func (h *CompatibilityHandler) CheckAnswerV1(c *gin.Context) {
	...
	_ = h.db.Create(&answerRec).Error
	h.updateKnowledgeMapFromQuestion(req.StudentID, question, isCorrect)
...
func (h *CompatibilityHandler) GetBreakpointV1(c *gin.Context) {
	...
	apiresp.OK(c, "请求成功", gin.H{"courseId": courseID, "pageNum": lastPage})
}
...
func (h *CompatibilityHandler) UpdateBreakpointV1(c *gin.Context) {
	...
	apiresp.OK(c, "ok", gin.H{"pageNum": page})
}
...
func (h *CompatibilityHandler) SaveNoteV1(c *gin.Context) {
	...
	apiresp.OK(c, "保存成功", gin.H{"noteId": note.ID, "nodeId": nodeID})
}
...
func (h *CompatibilityHandler) GetStudentNotesV1(c *gin.Context) {
	...
	apiresp.OK(c, "请求成功", gin.H{"items": items, "total": total, "page": pageNum, "pageSize": pageSize, "totalPages": (total + int64(pageSize) - 1) / int64(pageSize)})
}
...
func (h *CompatibilityHandler) AddFavoriteV1(c *gin.Context) {
	...
	if err == nil {
		...
		apiresp.OK(c, "请求成功", gin.H{"favoriteId": existing.ID, "message": "已收藏"})
		return
	}
	if err := h.db.Create(&favorite).Error; err != nil {
		apiresp.Internal(c, "收藏失败", "")
		return
	}
	apiresp.OK(c, "请求成功", gin.H{"favoriteId": favorite.ID})
}
...
func (h *CompatibilityHandler) GetFavoritesV1(c *gin.Context) {
	...
	apiresp.OK(c, "请求成功", gin.H{"items": payloadItems, "total": total, "page": page, "pageSize": pageSize, "totalPages": (total + int64(pageSize) - 1) / int64(pageSize)})
}
...
func (h *CompatibilityHandler) GeneratePracticeV1(c *gin.Context) {
	...
	task := model.PracticeTask{
		...
		QuestionIDs: string(questionJSON),
		Status:      "pending",
	}
	if err := h.db.Create(&task).Error; err != nil {
		apiresp.Internal(c, "生成练习失败", "")
		return
	}
	apiresp.OK(c, "请求成功", gin.H{"taskId": task.ID, "questions": sanitizePracticeQuestions(questions), "questionCount": len(questions)})
}
...
func (h *CompatibilityHandler) SubmitPracticeV1(c *gin.Context) {
	...
	if err := h.db.Where("id = ? AND user_id = ?", req.TaskID, req.StudentID).First(&task).Error; err != nil {
		apiresp.NotFound(c, "练习任务不存在", "")
		return
	}
	...
	if err := h.db.Where("task_id = ? AND user_id = ?", req.TaskID, req.StudentID).First(&existing).Error; err == nil {
		apiresp.OK(c, "请求成功", gin.H{
			...
			"details":      parsePracticeAttemptDetails(existing.AnswersJSON),
		})
		return
	}

	questions := make([]practiceQuestionPayload, 0)
	if err := json.Unmarshal([]byte(task.QuestionIDs), &questions); err != nil {
		apiresp.Internal(c, "练习数据损坏", "")
		return
	}
	...
	attempt := model.PracticeAttempt{
		...
		AnswersJSON:  string(detailsJSON),
	}
	if err := h.db.Create(&attempt).Error; err != nil {
		apiresp.Internal(c, "提交失败", "")
		return
	}
	apiresp.OK(c, "请求成功", gin.H{"attemptId": attempt.ID, "score": score, "correctCount": correctCount, "totalCount": totalCount, "details": details})
}
...
func (h *CompatibilityHandler) GetPracticeTaskV1(c *gin.Context) {
	...
	studentID := strings.TrimSpace(c.Query("studentId"))
	if taskID == "" || studentID == "" {
		apiresp.BadRequest(c, "缺少 taskId 或 studentId", "")
		return
	}

	var task model.PracticeTask
	if err := h.db.Where("id = ? AND user_id = ?", taskID, studentID).First(&task).Error; err != nil {
		apiresp.NotFound(c, "练习任务不存在", "")
		return
	}
	...
	if err := h.db.Where("task_id = ? AND user_id = ?", task.ID, studentID).First(&attempt).Error; err == nil {
		payload["attempt"] = gin.H{
			"attemptId":    attempt.ID,
			"score":        attempt.Score,
			"correctCount": attempt.CorrectCount,
			"totalCount":   attempt.TotalCount,
			"details":      parsePracticeAttemptDetails(attempt.AnswersJSON),
			"submittedAt":  attempt.CreatedAt,
		}
	}

	apiresp.OK(c, "请求成功", payload)
}
...
func (h *CompatibilityHandler) GetPracticeHistoryV1(c *gin.Context) {
	...
	for _, task := range tasks {
		var attempt model.PracticeAttempt
		_ = h.db.Where("task_id = ? AND user_id = ?", task.ID, studentID).First(&attempt).Error
		var qs []practiceQuestionPayload
		_ = json.Unmarshal([]byte(task.QuestionIDs), &qs)
		qCount := len(qs)
	...
	apiresp.OK(c, "请求成功", gin.H{"items": items, "total": total, "page": page, "pageSize": pageSize})
}
...
func (h *CompatibilityHandler) RetryWrongQuestionV1(c *gin.Context) {
	...
	task := model.PracticeTask{
		...
		CourseID:    question.CourseID,
		NodeID:      question.NodeID,
		PageIndex:   question.PageNum,
		Difficulty:  question.Difficulty,
		QuestionIDs: string(taskQuestionsJSON),
		Status:      "pending",
	}
	if err := h.db.Create(&task).Error; err != nil {
		apiresp.Internal(c, "生成重做任务失败", "")
		return
	}
	apiresp.OK(c, "请求成功", gin.H{"taskId": task.ID, "questions": sanitizePracticeQuestions(taskQuestions)})
}
...
func (h *CompatibilityHandler) ExplainNodeV1(c *gin.Context) {
	...
	apiresp.OK(c, "请求成功", gin.H{"nodeId": nodeID, "explanation": resp.Answer, "sourcePage": resp.SourcePage, "sourceExcerpt": resp.SourceExcerpt})
}
...
func (h *CompatibilityHandler) GetStudyStatsV1(c *gin.Context) {
	...
	apiresp.OK(c, "请求成功", gin.H{"studentId": studentID, "courseId": courseID, "focusScore": focusScore, "totalQuestions": totalQuestions, "weakPoints": weakPoints, "noteCount": countStudentNotes(h.db, studentID, courseID)})
}
...
func (h *CompatibilityHandler) CreateReviewPlanV1(c *gin.Context) {
	...
	if !ok {
		apiresp.BadRequest(c, "frequency 仅支持 daily / weekly / monthly", "")
		return
	}
	...
	apiresp.OK(c, "请求成功", normalizeReviewPlanPayload(plan))
}
...
func (h *CompatibilityHandler) GetReviewPlansV1(c *gin.Context) {
	...
	apiresp.OK(c, "请求成功", payloadPlans)
}
...
func (h *CompatibilityHandler) AddReviewPlanItemV1(c *gin.Context) {
	...
	if err := h.db.Create(&item).Error; err != nil {
		apiresp.Internal(c, "添加复习项失败", "")
		return
	}
	apiresp.OK(c, "请求成功", item)
}
...
func (h *CompatibilityHandler) GetReviewPlanItemsV1(c *gin.Context) {
	...
	apiresp.OK(c, "请求成功", payloadItems)
}
...
func (h *CompatibilityHandler) UpdateReviewPlanItemV1(c *gin.Context) {
	...
	apiresp.OK(c, "请求成功", normalizeReviewPlanItemPayload(item))
}
...
func (h *CompatibilityHandler) ensureKnowledgePointID(question model.Question) string {
	...
	if explicitID != "" {
		...
		point = model.KnowledgePoint{
			...
			CourseID:             courseID,
			SourceTeachingNodeID: strings.TrimSpace(question.NodeID),
...
func (h *CompatibilityHandler) ensureQuestionRecord(task model.PracticeTask, payload practiceQuestionPayload) (model.Question, error) {
	...
	question = model.Question{
		...
		PageNum:           defaultInt(payload.PageNum, task.PageIndex),
